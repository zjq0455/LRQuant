    ...
    for i in range(len(layers)):
        ...
        for name, module in layer.named_modules():
            if isinstance(module,torch.nn.Linear):
                kurtosis_value += scipy.stats.kurtosis(module.weight.data.detach().cpu().flatten().numpy())
                # print(kurtosis_value)

        layers[i] = layer.cpu()
        block_kurtosis_value[kurtosis_value/7] = i
        del layer
        torch.cuda.empty_cache()
...
if __name__ == "__main__":
    ...
    sensitive_ration = (robust_ratio / 3) * 4
